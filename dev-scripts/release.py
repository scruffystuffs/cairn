...
try:
    ...
    print("Must install tomlkit and requests into your python environment")
    sys.exit(1)
...
class Github:
    ...
    def _wait_for_workflow_run(self, run_id: int):
        ...
        for _ in timeout_iter(TIMEOUT):
            ...
            if not status == "completed":
                eprint(
                    f"Workflow not completed, current status: {status},",
                    f"sleeping for {WORKFLOW_POLL_SECONDS} seconds...",
                )
    ...
    def _get_run_id_by(self, func: Callable[[dict], bool]) -> int:
        backoff = 1
        hold_with_sticky(
            "Delaying run_id fetch to avoid conflicts", WORKFLOW_FETCH_DELAY
        )
